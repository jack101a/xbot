...
start_services() {
    ...
    echo -e "  🌐 ${BOLD}Dashboard UI (Local):${NC}    ${CYAN}http://localhost:${DASHBOARD_PORT}${NC} (or http://192.168.0.200:${DASHBOARD_PORT})"
    echo -e "  🔌 ${BOLD}Backend API (Local):${NC}     ${CYAN}http://localhost:${BACKEND_PORT}${NC} (or http://192.168.0.200:${BACKEND_PORT})"
...
stop_services() {
    ...
    pkill -9 -f "uvicorn xbot.main:app.*${BACKEND_PORT}" 2>/dev/null || true
...
check_status() {
    ...
    echo ""
    echo -e "${CYAN}💻 Local Services (Port ${BACKEND_PORT} Backend | Port ${DASHBOARD_PORT} Dashboard):${NC}"
