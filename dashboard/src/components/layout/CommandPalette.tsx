...
export function CommandPalette() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-6 sm:pt-[15vh] px-3 sm:px-6 pb-6">
      <div
        className="fixed inset-0 bg-slate-950/60 backdrop-blur-xs transition-opacity"
        onClick={() => setCommandPaletteOpen(false)}
      />
      <div
        className="relative w-full max-w-xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden z-10 flex flex-col max-h-[80dvh] sm:max-h-[70vh] animate-in fade-in zoom-in-95 duration-100"
      ...
      >
        <div className="flex items-center px-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
          ...
          <button
            onClick={() => setCommandPaletteOpen(false)}
            className="sm:hidden p-2 min-h-[40px] min-w-[40px] flex items-center justify-center text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 active:scale-90 transition"
