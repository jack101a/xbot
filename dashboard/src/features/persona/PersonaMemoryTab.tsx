      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            Configure authentic tone of voice, worldview orientations, linguistic code-switching, and daily diary logs.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:flex items-center gap-2 w-full sm:w-auto">
          <button
            ...
            className="flex items-center justify-center gap-1.5 px-3.5 py-2.5 rounded-xl text-xs font-semibold border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition shadow-sm w-full sm:w-auto"
          >
          ...
          <button
            ...
            disabled={saving}
            className="flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl text-xs font-bold bg-sky-600 hover:bg-sky-700 text-white shadow-md shadow-sky-600/20 transition disabled:opacity-50 w-full sm:w-auto"
          >
      ...
      <div className="overflow-x-auto no-scrollbar flex gap-1.5 p-1 rounded-xl bg-slate-100 dark:bg-slate-800/60 w-full sm:max-w-3xl scroll-smooth">
        {(["identity", "boundaries", "stances", "topics", "diary", "learned"] as const).map((tab) => (
          <button
            ...
            className={`whitespace-nowrap flex-shrink-0 flex-1 min-w-[100px] min-h-[44px] py-2.5 px-3 rounded-lg text-xs font-semibold transition-all text-center active:scale-[0.98] ${
              subSection === tab
