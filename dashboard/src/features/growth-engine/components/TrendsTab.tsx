...
import { 
  Sparkles, TrendingUp, RefreshCw, Send, ArrowRight, Activity
} from "lucide-react";
import { useTrends } from "../hooks/useTrends";
import { useAppStore } from "@/store/useAppStore";
import { InstantTrendSection } from "./InstantTrendSection";
...
export function TrendsTab({ profileId }: { profileId: string }) {
  ...
  return (
    <div className="space-y-6">
      {/* Unification Notice Banner */}
      <div className="p-4 rounded-xl bg-gradient-to-r from-sky-500/10 via-sky-500/5 to-transparent border border-sky-500/30 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-sky-500" />
            <h4 className="text-xs font-bold uppercase tracking-wider text-sky-600 dark:text-sky-400">
              Unified into AI Campaign Studio
            </h4>
          </div>
          <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
            Trend Radar and Instant Trend have merged! Run multi-day campaigns (12h to 7d) with authentic X official media extraction directly inside AI Campaign Studio.
          </p>
        </div>
        <button
          onClick={() => setActiveTab("campaigns")}
          className="px-4 py-2 rounded-xl bg-sky-600 hover:bg-sky-700 text-white text-xs font-bold flex items-center justify-center gap-1.5 transition flex-shrink-0 shadow-md shadow-sky-600/20"
        >
      ...
      <InstantTrendSection profileSlug="test_profile1" />
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Monitors live tech news & RSS feeds, scores alignment to your persona, and drafts timely commentary takes.
            </p>
          </div>
          <button
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="px-3 py-1 rounded-md bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold flex items-center justify-center gap-1 transition disabled:opacity-50"
                            >
