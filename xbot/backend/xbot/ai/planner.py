...
class PlannedAction(BaseModel):
    type: Literal["post", "reply", "like", "retweet", "quote", "follow", "unfollow", "browse", "search", "scrape_trends", "scrape_metrics", "unfollow_non_followers", "follow_engagers", "poll"]
    target: str | None = Field(default=None, description="tweet_url, username, query, or null")
    content: str | None = Field(default=None, description="text content for post/reply/quote, null otherwise")
...
async def plan_session(
    ...
    # 2. Format feed snapshot
    feed_str = "No active feed snapshot available."
    if feed_snapshot:
        feed_lines = []
        for i, tweet in enumerate(feed_snapshot[:20]):
    ...
    user_prompt += (
        "\n\n## Instructions\n"
        "Based on your state, recent history, active memories, strategy, and the current feed snapshot, "
        "plan your next session by returning a list of prioritize actions.\n"
        "Strictly adhere to your writing style, interests (will not discuss certain topics), and rules of engagement.\n"
        "Return a valid JSON object matching this schema:\n"
        "{\n"
        "  \"session_plan\": {\n"
        "    \"mood\": \"current mood/energy level\",\n"
        "    \"reasoning\": \"overall rationale for chosen actions\",\n"
        "    \"actions\": [\n"
        "      {\n"
        "        \"type\": \"post | reply | like | retweet | quote | follow | unfollow | browse | search | scrape_trends | scrape_metrics | unfollow_non_followers | follow_engagers\",\n"
        "        \"target\": \"tweet_url or username or search_query or null. For follow_engagers, must be a tweet_url.\",\n"
        "        \"content\": \"content text if composing a post/reply/quote, null otherwise\",\n"
