...
async def generate_content_for_topic(
    ...
    """Synthesizes and stages a post/thread/poll/visual spec for a researched topic via BrowserPort."""
    pkg = _get_pkg()
    ...
    # Automatically post live to X via BrowserPort if container is provided
    if container is not None:
        try:
            if content_record.content_type in (ContentType.POLL, "poll"):
                ...
                await c.browser.execute(post_req)
        except Exception as e:
            logger.warning("TrendGenerator: Direct publish encountered non-fatal error: %s", e)
...
async def run_trend_generator_for_profile(
    ...
    """Generates content from pending researched topics for a profile via BrowserPort."""
    pkg = _get_pkg()
    c = container or get_container()
    stmt = (
...
async def _run_trend_generator_async(container: Container | None = None) -> dict[str, Any]:
    pkg = _get_pkg()
    guard = CentralGuard()
    c = container or get_container()
    ...
    async with AsyncSessionLocal() as db:
        ...
        for profile in profiles:
            try:
                res = await pkg.run_trend_generator_for_profile(db, profile, guard, container=c)
