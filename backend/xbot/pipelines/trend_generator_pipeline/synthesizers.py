...
async def synthesize_visual_topic(
    ...
    if not visual_spec:
        ...
        clean_top = topic.topic.strip().rstrip(".").capitalize()
        visual_spec = VisualPostSpec(
            tweet_copy=f"{clean_top}." if len(clean_top) <= 120 else f"{clean_top[:117]}...",
            image_prompt=f"Cinematic 4:5 visual breakdown of {topic.topic}, dark aesthetic (#0D1117), 8k realism.",
            aspect_ratio="4:5",
    ...
    if not media_to_attach:
        try:
            ...
            if gen_img:
                img_path = gen_img if isinstance(gen_img, str) else gen_img.get("file_path")
                if img_path and os.path.exists(img_path):
                    media_to_attach = [img_path]
    ...
    content_record = Content(
        ...
        ai_metadata={
            "topic": topic.topic,
            "topic_tag": extract_topic_tag(topic.topic),
            "archetype": "VISUAL",
