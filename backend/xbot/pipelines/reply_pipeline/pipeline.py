...
async def run_reply_pipeline_for_profile(
    ...
    """Executes full unified reply cycle for a profile using BrowserPort."""
    pkg = _get_pkg()
    ...
    sniper_count = await pkg.execute_kol_sniper_replies(db, profile, guard, max_replies=2, container=c)
    ...
    if sniper_count < max_total_replies:
        sentinel_count = await pkg.execute_fast_response_replies(
            db, profile, guard, max_replies=(max_total_replies - sniper_count), container=c
        )
    ...
    if remaining_budget > 0:
        feed_count = await pkg.execute_feed_replies(db, profile, guard, max_replies=remaining_budget, container=c)
...
async def _run_reply_pipeline_async(container: Container | None = None) -> dict[str, Any]:
    pkg = _get_pkg()
    guard = CentralGuard()
    c = container or get_container()
    ...
    async with AsyncSessionLocal() as db:
        ...
        for profile in profiles:
            try:
                res = await pkg.run_reply_pipeline_for_profile(db, profile, guard, container=c)
