...
async def _search_and_scrape_x(query: str, profile_slug: str, search_filter: str = "top") -> list[dict[str, Any]]:
    ...
    try:
        ...
        res = await asyncio.to_thread(get_browser_job_result, job_id, 75.0)
        if res and res.get("status") == "success":
            return res.get("results", [])
    except Exception as e:
        logger.warning("OnDemandCampaign: Search query '%s' (filter=%s) encountered error: %s", query, search_filter, e)
    return []
