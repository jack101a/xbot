...
async def publish_campaign_deliverables(
    ...
    """
    Publishes selected deliverables either instantly (via BrowserPort) or
    schedules them by setting status = APPROVED.
    """
    if db is None:
        async with AsyncSessionLocal() as session:
            return await publish_campaign_deliverables(campaign_id, content_ids, mode, interval_minutes, session, container=container)
    ...
    for idx, rec in enumerate(records):
        ...
        if mode == "instant":
            ...
            try:
                ...
                logger.warning("Could not publish direct browser request for deliverable %s: %s", rec.id, q_err)
