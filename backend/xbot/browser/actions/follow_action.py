...
class UnfollowUser(BaseAction):
    ...
    async def execute(self, page: Page, username: str) -> bool:
        try:
            ...
            # STRICT MUTUAL SAFETY CHECK: Never unfollow someone who follows us back!
            follows_you_badge = await page.query_selector(
                '[data-testid="userFollowIndicator"], span:has-text("Follows you"), div:has-text("Follows you")'
            )
            if follows_you_badge:
                logger.info(
                    "ABORT UNFOLLOW: User @%s follows us back ('Follows you' badge detected). Protecting mutual relationship!",
                    clean,
                )
                return {
                    "status": "skipped",
                    "reason": "mutual_follower",
                    "message": f"User @{clean} follows us back. Mutual relationship protected.",
                    "follows_you": True,
                }
            ...
            if not unfollow_btn:
                logger.info("User @%s is already not followed (no Following button found).", clean)
                return True
            ...
            try:
                confirm_btn = await page.wait_for_selector(
                    "[data-testid='confirmationSheetConfirm'], [data-testid='confirmationSheetDialog'] button, div[role='dialog'] button:has-text('Unfollow'), button:has-text('Unfollow')",
                    timeout=4000
                )
                if confirm_btn:
                ...
                logger.debug("No separate confirmation modal needed or detected for @%s", clean)
