            ...
            for _ in range(max_scrolls):
                ...
                for el in tweet_elements:
                    ...
                    if text_el:
                        ...
                        try:
                            img_els = await el.query_selector_all('[data-testid="tweetPhoto"] img, div[aria-label="Image"] img')
                            ...
                            video_el = await el.query_selector('[data-testid="videoComponent"], [data-testid="videoPlayer"], video, div[aria-label*="video" i]')
                            if video_el:
...
class SearchQuery(BaseAction):
    ...
    async def execute(
        ...
        try:
            ...
            search_url = f"https://x.com/search?q={quote_plus(query)}{cat_param}"
            logger.info("Executing X search: %s (url=%s)", query, search_url)
            ...
            results = await browser.execute(page, max_scrolls=4, navigate_home=False)
