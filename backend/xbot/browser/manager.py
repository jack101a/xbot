...
class BrowserManager:
    ...
    def acquire_lock(self, profile_slug: str, timeout_seconds: int = 1800, global_concurrency: bool = False) -> bool:
        """
        ...
        global_key = "lock:browser:host_singleton"

        for _ in range(15):
            if global_concurrency:
                if not self._redis_client.set(global_key, profile_slug, ex=timeout_seconds, nx=True):
                    holder = self._redis_client.get(global_key)
                    if holder and (holder == profile_slug or (isinstance(holder, bytes) and holder.decode() == profile_slug)):
                        pass
                    else:
                        time.sleep(2.0)
                        continue
    ...
    def release_lock(self, profile_slug: str) -> None:
        """
        Releases the execution lock for a profile and the host singleton lock.
        """
        lock_key = f"lock:browser:{profile_slug}"
        ...
        try:
            holder = self._redis_client.get(global_key)
            if holder and (holder == profile_slug or (isinstance(holder, bytes) and holder.decode() == profile_slug)):
                self._redis_client.delete(global_key)
        except Exception:
            pass
    ...
    async def get_context(
        ...
        # Clean up stale Chromium Singleton locks only if the owning process is dead
            ...
            for stale_name in ("SingletonLock", "SingletonSocket", "SingletonCookie"):
                ...
                if stale_p.exists() or stale_p.is_symlink():
                    try:
                        ...
                        pass
            # Clean up stale Service Worker and cache to prevent X splash screen freeze
            import shutil
            for cache_subdir in ("Service Worker", "Cache", "Code Cache", "GPUCache", "ShaderCache"):
                cache_p = user_data_dir / "Default" / cache_subdir
                if cache_p.exists():
                    try:
                        shutil.rmtree(str(cache_p), ignore_errors=True)
                    except Exception:
                        pass
        else:
            logger.warning("Chromium Singleton process is still running for %s; leaving locks intact.", profile_slug)
