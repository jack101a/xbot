...
async def get_f4f_candidates(
    ...
    status_order = case((FollowCandidate.status == "discovered", 0), else_=1)
    query = query.order_by(status_order, FollowCandidate.reciprocity_score.desc()).limit(limit)
...
async def execute_f4f_follow(
    ...
    success = res.status in ("success", "followed") or (res.action_result and res.action_result.status == "success")
    ...
    return {
        ...
        "message": f"Successfully followed @{clean_handle} on X!" if success else (res.error or f"Failed to follow @{clean_handle}."),
        "target_handle": clean_handle,
    }
