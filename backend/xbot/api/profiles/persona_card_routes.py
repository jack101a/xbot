...
async def update_profile_persona(
    ...
    try:
        ...
        current_data.update(persona_in)
        validated_persona = Persona.model_validate(current_data)
        save_persona(profile_dir, validated_persona)
    except Exception as e:
        logger.warning("Persona validation failed during update, writing merged dict: %s", e)
        with persona_file.open("w", encoding="utf-8") as f:
            yaml.dump(current_data if current_data else persona_in, f)
