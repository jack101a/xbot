...
async def get_system_config() -> dict[str, Any]:
    ...
    if env_path and env_path.exists():
        try:
            ...
            logger.warning("Error refreshing settings from .env: %s", e)
    return {
...
async def get_system_resources() -> dict[str, Any]:
    """Retrieves real-time RAM, CPU, and process resource utilization for XBot Pro."""
    try:
        ...
        monitor = ResourceMonitor(project_root=root)
        snap = monitor.collect_snapshot()
        return asdict(snap)
    except Exception as e:
        logger.exception("Failed to collect system resources: %s", e)
