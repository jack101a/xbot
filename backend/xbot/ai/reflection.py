...
class ReflectionEngine:
    ...
    async def reflect_and_update(
        ...
        # Retrieve recent episodic memories, recent posted content, and recent diary session logs
        recent_memories = memory_mgr.retrieve_memories(recency_limit=40)
        recent_diary_blocks = diary_mgr.get_recent_session_blocks(limit=5)
        stmt_posts = (
            select(Content)
            .where(
        ...
        posts_summary = "\n".join(recent_posts)
        diary_summary = "\n\n".join(recent_diary_blocks) if recent_diary_blocks else "No recent diary entries available."
        ...
        system_prompt = (
            ...
            f"Your job is to review recent interactions, memories, posted tweets, audience performance metrics, and the persona's recent subjective diary sessions, and synthesize an updated LearnedState across 7 categories:\n"
            ...
            f"=== ANTI-HALLUCINATION & DIARY VALIDATION DIRECTIVE ===\n"
            f"- DIARY ENTRIES ARE SUBJECTIVE HYPOTHESES; AUDIENCE FEEDBACK METRICS ARE OBJECTIVE GROUND TRUTH.\n"
            f"- Cross-examine diary claims against audience metrics. If the diary claims an angle or tone worked well ('How it went'), "
            f"verify if the performance metrics corroborate it. If metrics show zero or poor engagement, DISREGARD the claim.\n"
            f"- ONLY promote behavioral adaptations, writing patterns, or tactics into 'habits' or 'characteristics' if backed by tangible audience resonance (likes, reposts, positive replies).\n"
            f"- High-signal timeline observations or hypotheses from 'What I learned' or 'Thoughts for next time' in the diary can seed new topics in 'emerging_topics' for future testing.\n"
            f"- If the diary documents creative fatigue, cynicism, or tonal shifts that correlate with audience resonance, reflect them in 'evolving_nuances'.\n\n"
        ...
        user_prompt = (
            ...
            f"Audience Feedback & Tweet Performance (Ground Truth):\n{perf_summary}\n\n"
            f"Recent Persona Diary Sessions (Subjective Context & Hypotheses):\n{diary_summary}\n\n"
