...
def _build_clean_creator_prompt(
    ...
    if post_type == "thread":
        ...
        sections.append(
            ...
            "- NEVER use numbered lists (1. 2. 3.) or bullet points in single posts. Write flowing, organic text.\n"
    ...
    # 7. Worldview, Stances, and Linguistic Directives
    worldview_block = build_worldview_prompt_section(
        persona,
        context_text=f"{topic} {context_summary or ''}",
        is_reply=(post_type != "post"),
    )
    if worldview_block:
        sections.append(f"## 🌐 Persona Worldview, Stance & Style Directives\n{worldview_block}")
