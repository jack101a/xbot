...
async def fetch_x_trends(
    ...
    """
    Fetches real-time trending topics EXCLUSIVELY from X (Twitter):
    1. Scrapes X Explore / Trending tab via BrowserPort.
    2. Scrapes viral tweets from X Home/For-You Feed via BrowserPort.
    Strictly filters out politics and sports score spam.
    """
    ...
    try:
        trend_req = BrowserRequest(
            ...
            params={"limit": max_total},
            timeout_seconds=40,
        )
        res = await c.browser.execute(trend_req)
        raw_trends = []
        if res.status == "success":
            if res.scrape and res.scrape.raw.get("trends"):
                raw_trends = res.scrape.raw["trends"]
            elif res.action_result and res.action_result.raw.get("trends"):
                raw_trends = res.action_result.raw["trends"]
        ...
        for t in raw_trends:
            ...
            context = t.get("context") or t.get("category") or "Trending on X"
            volume = t.get("volume") or t.get("tweet_count") or ""
            summary = f"{context} with {volume} posts" if volume else context
            items.append(
                TrendItem(
                    id=item_id,
                    title=name,
        ...
        logger.warning("Failed to fetch X Explore trends: %s", ex)

    # 2. Source B: X Feed Viral Posts (if needed to reach max_total)
    if len(items) < max_total:
        try:
            ...
            res = await c.browser.execute(feed_req)
            raw_tweets = []
            if res.status == "success":
                if res.scrape and res.scrape.tweets:
                    raw_tweets = [tw.model_dump() for tw in res.scrape.tweets]
                elif res.scrape and res.scrape.raw.get("tweets"):
                    raw_tweets = res.scrape.raw["tweets"]
                elif res.action_result and res.action_result.raw.get("tweets"):
                    raw_tweets = res.action_result.raw["tweets"]

            for tw in raw_tweets:
                text = (tw.get("text") or "").strip()
                if not text or len(text) < 30 or BANNED_POLITICS_REGEX.search(text):
                    continue
                first_line = text.split("\n")[0].strip()
                title = first_line[:100] if len(first_line) > 15 else text[:100]
                item_id = hashlib.sha256(f"x_feed:{title.lower()}".encode()).hexdigest()[:16]
                if item_id in seen_hashes:
                    continue
                seen_hashes.add(item_id)
                author = tw.get("author") or tw.get("handle") or "creator"
                items.append(
                    TrendItem(
                        id=item_id,
                        title=title,
                        summary=text[:280],
                        source_url=tw.get("tweet_url") or tw.get("url") or f"https://x.com/{author}",
                        source_name="X (Feed)",
                        published_at=datetime.now(timezone.utc).isoformat(),
                    )
                )
            ...
            logger.warning("Failed to fetch X Feed viral posts: %s", ex)
