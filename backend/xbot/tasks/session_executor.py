        ...
        try:
            ...
            if is_mock:
                ...
                if p_action.type == "post" and p_action.content:
                    success = await handle_post_action(db, profile_id, profile_slug, p_action, db_action, session, page, config, persona, t_start, browser=browser)
                    if db_action.status == ActionStatus.SKIPPED:
                        continue
                elif p_action.type in ("poll", ActionType.POLL):
                    success = await handle_poll_action(db, profile_id, profile_slug, p_action, db_action, session, page, config, manager, t_start, browser=browser)
                elif p_action.type == "thread":
                    success = await handle_thread_action(db, profile_id, profile_slug, p_action, db_action, session, page, config, manager, t_start, browser=browser)
                    ...
                    success = await handle_reply_action(db, profile_id, profile_slug, p_action, db_action, page, manager, tweet_url, browser=browser)
                    if db_action.status == ActionStatus.SKIPPED:
                        continue
                elif p_action.type == "quote":
                    success = await handle_quote_action(db, profile_id, p_action, db_action, page, persona, tweet_url, is_mock, browser=browser, profile_slug=profile_slug)
                    if db_action.status == ActionStatus.SKIPPED:
                        continue
                elif p_action.type == "follow":
                    success = await handle_follow_action(db, profile_id, p_action, username_target, page, browser=browser, profile_slug=profile_slug)
                else:
                    success = await handle_simple_action(p_action, tweet_url, username_target, page, browser=browser, profile_slug=profile_slug)
