...
from xbot.ai.trend_radar import fetch_x_trends, fetch_multi_source_trends
        ...
        async with tasks.AsyncSessionLocal() as db:
            ...
            for profile in active_profiles:
                ...
                try:
                    ...
                    keywords = []
                    if persona.interests and persona.interests.primary:
                        keywords = list(persona.interests.primary)

                    # 3. Fetch trends EXCLUSIVELY from X (Twitter Explore & Viral Feed)
                    try:
                        trends = await fetch_x_trends(
                            profile_slug=profile_slug,
                            keywords=keywords,
                            max_total=12,
                        )
                    except Exception as ex:
                        logger.warning("Failed to fetch X trends for profile %s: %s", profile_slug, ex)
                        errors.append(f"{profile_slug} X trend fetch: {ex}")
                        continue
                    ...
                    for item in trends:
                        ...
                        if eval_result.is_relevant and eval_result.relevance_score >= 0.70 and eval_result.optimized_post and items_staged < 3:
                            ...
                            if eval_result.thread_items and len(eval_result.thread_items) >= 2:
