...
async def handle_thread_action(
    ...
    if not raw_tweets or len(raw_tweets) < 2:
        ...
        gen_thread = await generate_thread(topic=topic or "Creator Strategy & Growth Breakdown", persona=p_obj, num_tweets=None)
    ...
    if page:
        ...
        resp = await browser.execute(BrowserRequest(
            ...
            params={"tweets": raw_tweets},
        ))
        success = resp.status == "success"
        res = (resp.action_result.raw if resp.action_result else None) or {}
    else:
        success = False
        res = {}
