...
async def _run_session_async(profile_id_str: str) -> dict[str, Any]:
    ...
    async with tasks.AsyncSessionLocal() as db:
        ...
        # 3. Setup BrowserPort and Profile Configuration
        from xbot.container import get_container
        container = get_container()
        browser = getattr(container, "browser", None)

        manager = tasks.BrowserManager(base_profile_dir=settings.BASE_PROFILE_DIR)

        # Ensure single session per profile
        ...
        try:
            ...
            if not is_mock and not browser:
                try:
                    ...
                    logger.debug("Direct browser context not initialized in session runner: %s; falling back to queue.", ctx_err)

            # Feed & Opportunity snapshot via Research Lane / page
            ...
            # 5. Execute planned actions via Action Lane / page
            ...
            await audit_reciprocity_and_prune(db, profile_id, profile_slug, session.id, page=page, is_mock=is_mock, browser=browser)
            await audit_and_prune_misaligned_actions(db, profile_id, page=page, is_mock=is_mock, browser=browser, profile_slug=profile_slug)
            clean_temp_storage_and_logs()
