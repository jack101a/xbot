...
async def handle_reply_action(
    ...
    if page:
        ...
        ctx_resp = await browser.execute(BrowserRequest(
            profile_slug=profile_slug,
            action=BrowserActionType.SCRAPE_TWEET_CONTEXT,
            params={"tweet_url": tweet_url},
        ))
        if ctx_resp.status == "success":
            live_ctx = (ctx_resp.scrape.raw if ctx_resp.scrape and ctx_resp.scrape.raw else None) or (ctx_resp.action_result.raw.get("context") if ctx_resp.action_result and ctx_resp.action_result.raw else None) or {}
    ...
    if target_to_follow:
        ...
        if page:
            ...
            resp = await browser.execute(BrowserRequest(
                ...
                params={"username": clean_f_handle},
            ))
...
async def handle_simple_action(
    ...
    if browser:
        if p_action.type == "like":
            resp = await browser.execute(BrowserRequest(profile_slug=profile_slug, action=BrowserActionType.LIKE, params={"tweet_url": tweet_url}))
            return resp.status == "success"
        elif p_action.type == "retweet":
            resp = await browser.execute(BrowserRequest(profile_slug=profile_slug, action=BrowserActionType.REPLY, params={"tweet_url": tweet_url, "retweet": True}))
            return resp.status == "success"
        elif p_action.type == "search" and p_action.target:
            resp = await browser.execute(BrowserRequest(profile_slug=profile_slug, action=BrowserActionType.SEARCH, params={"query": p_action.target}))
            return resp.status == "success"
        elif p_action.type == "browse":
            resp = await browser.execute(BrowserRequest(profile_slug=profile_slug, action=BrowserActionType.SCRAPE_FEED, params={"scrolls": 1}))
            return resp.status == "success"
        elif p_action.type == "unfollow" and (username_target or p_action.target):
            clean_u = (username_target or p_action.target).lstrip("@")
            resp = await browser.execute(BrowserRequest(profile_slug=profile_slug, action=BrowserActionType.UNFOLLOW, params={"username": clean_u}))
            return resp.status == "success"
        elif p_action.type == "scrape_trends":
            resp = await browser.execute(BrowserRequest(profile_slug=profile_slug, action=BrowserActionType.SCRAPE_TRENDING))
            return resp.status == "success"
        elif p_action.type == "unfollow_non_followers":
            resp = await browser.execute(BrowserRequest(profile_slug=profile_slug, action=BrowserActionType.UNFOLLOW, params={"limit": 10}))
            return resp.status == "success"
        elif p_action.type == "follow_engagers" and (tweet_url or p_action.target):
            resp = await browser.execute(BrowserRequest(profile_slug=profile_slug, action=BrowserActionType.FOLLOW, params={"tweet_url": tweet_url or p_action.target, "limit": 5}))
            return resp.status == "success"
