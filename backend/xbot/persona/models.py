...
class EntityStance(BaseModel):
    name: str = Field(..., description="Entity name, franchise, person, or concept (e.g. Christopher Nolan, One Piece, Apple, Corporate AI Slop)")
    category: str = Field("cinema", description="Category: cinema, tech, anime, culture, gaming, general")
    archetype: str = Field("loyalist", description="Stance archetype: loyalist, nemesis, nuanced_critic, guilty_pleasure, enthusiast, observer")
    sentiment_split: dict[str, int] = Field(
        default_factory=lambda: {"praise": 70, "analysis": 20, "critique": 10},
        description="Sentiment/orientation ratio percentage split (praise, analysis, critique)"
    )
    talking_points: list[str] = Field(default_factory=list, description="Core themes or angles for this entity")
    behavioral_rule: str = Field("", description="Specific behavioral rule or nuance when this entity is brought up")
    is_active: bool = Field(True, description="Whether this stance is actively applied")


class LanguageConfig(BaseModel):
    primary_language: str = Field("en", description="Default primary language (e.g. en, hi)")
    enable_hinglish: bool = Field(True, description="Enable Hinglish conversational code-switching")
    hinglish_mode: str = Field(
        "mirror_and_punchline",
        description="Code-switching mode: 'mirror_only' (switch only when parent/room is Hindi/Hinglish), 'mirror_and_punchline' (English setup + Hinglish punchline/banter), 'full_bilingual' (fluid mix)"
    )
    slang_register: str = Field(
        "urban_buff",
        description="Hinglish slang register: 'urban_buff' (yaar, scene, legit, sahi mein), 'casual_desi' (bhai, arre, kya baat hai), 'minimal' (rare subtle tone markers)"
    )
    enable_hindi_script: bool = Field(True, description="Allow replying in Devanagari Hindi when parent post is in Hindi")


class ExpressivenessConfig(BaseModel):
    emoji_mode: str = Field(
        "contextual_tone",
        description="Emoji mode: 'contextual_tone' (paralinguistic tone markers/irony), 'minimal' (rare 0-1), 'none' (strictly zero emojis)"
    )
    max_emojis: int = Field(2, description="Maximum emojis allowed per tweet/reply")
    allow_zero_emojis: bool = Field(True, description="Allow 0 emojis for deadpan, cynical, or analytical takes")
    reply_meme_rate: int = Field(25, description="Percentage likelihood of attaching a reaction GIF/meme when comedic timing fits")
    thread_media_rate: int = Field(60, description="Percentage target for attaching visual stills/diagrams to threads")
...
class Persona(BaseModel):
    ...
    stances: list[EntityStance] = Field(default_factory=list)
