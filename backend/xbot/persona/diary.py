...
"""
    ...
    def get_recent_session_blocks(self, limit: int = 5) -> list[str]:
        """
        Retrieves the most recent session blocks across recent diary files,
        ordered chronologically (oldest to newest) to preserve narrative progression.
        """
        if not self.diary_dir.exists():
            return []
        ...
        blocks: list[str] = []
        for file_path in diary_files[:3]:
            try:
                content = file_path.read_text(encoding="utf-8")
                parts = re.split(r"(?=^## Session \d+)", content, flags=re.MULTILINE)
                date_str = file_path.stem
                for part in reversed(parts):
                    trimmed = part.strip()
                    if trimmed.startswith("## Session"):
                        blocks.append(f"[{date_str}] {trimmed}")
                        if len(blocks) >= limit:
                            break
            except Exception as e:
                logger.error("Failed to parse sessions from %s: %s", file_path, e)
            if len(blocks) >= limit:
                break
