...
async def lifespan(app: FastAPI):
    ...
    # Background scheduler is handled by Celery Beat to prevent duplicate concurrent sessions.
    # Enable internal loop only if explicitly requested (e.g. running standalone without Celery).
    scheduler_task = None
    if settings.__dict__.get("ENABLE_INTERNAL_SCHEDULER", False):
        logger.info("Starting standalone background growth scheduler in FastAPI...")
        scheduler_task = asyncio.create_task(_background_growth_scheduler_loop())
    yield
    # Cancel scheduler on shutdown if running
    if scheduler_task:
...
# Set up CORS middleware (including Private Network Access for LAN IP access)
app.add_middleware(
    CORSMiddleware,
