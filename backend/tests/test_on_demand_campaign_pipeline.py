...
async def test_execute_on_demand_campaign_flow(tmp_path):
    ...
    async with TestingSessionLocal() as db:
        ...
        mock_thread_res = MagicMock(tweets=["GIVA controversy tweet 1", "Tweet 2", "Tweet 3", "Tweet 4"])
        mock_poll_res = MagicMock(question="Apple upgrade?", options=["Yes", "No"], duration_days=1, context_hook="Apple event", reasoning="Test")
        ...
        with patch("xbot.pipelines.on_demand_campaign_pipeline.plan_campaign_from_prompt", AsyncMock(return_value=mock_plan)), \
             ...
             patch("xbot.pipelines.on_demand_campaign_pipeline.generate_thread", AsyncMock(return_value=mock_thread_res)), \
             patch("xbot.pipelines.on_demand_campaign_pipeline.generate_poll", AsyncMock(return_value=mock_poll_res)), \
