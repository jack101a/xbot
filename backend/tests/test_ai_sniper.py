...
@pytest.mark.asyncio
async def test_generate_quote_take_success(
    sample_persona: Persona, sample_tweet: dict
) -> None:
    """Tests that generate_quote_take executes successfully and handles JSON markdown cleanup."""
    from xbot.ai.sniper import generate_quote_take
    from xbot.ai.sniper.verifier import QuoteTakeResult

    mock_json_content = """```json
    {
        "topic_understanding": "Analysis on modern distributed database bottlenecks.",
        "quote_text": "\\"Database bottlenecks are rarely CPU-bound; they are 99% I/O and locking.\\"",
        "gif_query": "mind blown",
        "reasoning": "High-value contrarian insight."
    }
    ```"""

    mock_choice = MagicMock()
    mock_choice.message = MagicMock(content=mock_json_content)
    mock_client = AsyncMock()
    mock_client.chat.completions.create.return_value = MagicMock(choices=[mock_choice])

    res = await generate_quote_take(
        persona=sample_persona,
        target_tweet=sample_tweet,
        client=mock_client,
    )

    assert isinstance(res, QuoteTakeResult)
    assert res.quote_text == "Database bottlenecks are rarely CPU-bound; they are 99% I/O and locking."
    assert res.gif_query == "mind blown"
    assert res.confidence == 1.0


@pytest.mark.asyncio
async def test_generate_quote_take_fallback_on_error(
    sample_persona: Persona, sample_tweet: dict
) -> None:
    """Tests that generate_quote_take returns empty quote_text fallback on LLM failure."""
    from xbot.ai.sniper import generate_quote_take
    from xbot.ai.sniper.verifier import QuoteTakeResult
    ...
    assert isinstance(res, QuoteTakeResult)
    assert res.quote_text == ""
    assert res.confidence == 0.0
    assert "exhausted" in res.reasoning.lower()
