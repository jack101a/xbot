...
async def test_execute_kol_sniper_replies_enriched_context():
    ...
    with patch("xbot.infra.browser.queued_adapter.enqueue_browser_job", side_effect=mock_enqueue_job):
        with patch("xbot.infra.browser.queued_adapter.get_browser_job_result") as mock_res:
...
async def test_execute_fast_response_replies_with_persona_and_gif():
    ...
    with patch("xbot.infra.browser.queued_adapter.enqueue_browser_job", side_effect=mock_enqueue_job):
        with patch("xbot.infra.browser.queued_adapter.get_browser_job_result") as mock_res:
            mock_res.side_effect = [{"status": "replied"}, {"status": "liked"}]
    ...
    with patch("xbot.pipelines.reply_pipeline.execute_kol_sniper_replies", AsyncMock(return_value=1)) as mock_kol:
        with patch("xbot.pipelines.reply_pipeline.execute_fast_response_replies", AsyncMock(return_value=1)) as mock_fast:
            with patch("xbot.pipelines.reply_pipeline.execute_feed_replies", AsyncMock(return_value=1)) as mock_feed:
                ...
                assert mock_kol.call_count == 1
                assert mock_kol.call_args.args[0] == mock_db
                assert mock_kol.call_args.args[1] == profile
                assert mock_kol.call_args.args[2] == mock_guard
                assert mock_kol.call_args.kwargs.get("max_replies") == 2

                assert mock_fast.call_count == 1
                assert mock_fast.call_args.args[0] == mock_db
                assert mock_fast.call_args.args[1] == profile
                assert mock_fast.call_args.args[2] == mock_guard
                assert mock_fast.call_args.kwargs.get("max_replies") == 2

                assert mock_feed.call_count == 1
                assert mock_feed.call_args.args[0] == mock_db
                assert mock_feed.call_args.args[1] == profile
                assert mock_feed.call_args.args[2] == mock_guard
                assert mock_feed.call_args.kwargs.get("max_replies") == 1
