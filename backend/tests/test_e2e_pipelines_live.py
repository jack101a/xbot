...
async def test_e2e_dynamic_sniper_room_reader_modalities():
    ...
    for mode, reply_text, gif_query in scenarios:
        ...
        with patch("xbot.ai.fact_grounder.ground_context_with_live_facts", AsyncMock(return_value="")):
...
async def test_e2e_trend_generator_4way_creation_matrix():
    ...
    async with AsyncSessionLocal() as db:
        ...
        mock_visual_spec = VisualPostSpec(
            topic="Late night debugging",
            tweet_copy="The duality of shipping on Friday.",
            image_prompt="4:5 vertical meme",
            aspect_ratio="4:5",
            format_type="MEME_IMAGE",
            target_simcluster="Technology",
            one_two_punch_strategy="Punchy take with visual punchline",
        )
        mock_poll = GeneratedPoll(
            question="Which database handles heavy writes best?",
            options=["PostgreSQL MVCC", "MySQL InnoDB", "SQLite (WAL mode)", "CockroachDB"],
            duration_days=1,
            context_hook="Edge databases comparison",
            reasoning="Sparks architecture debate",
        )
        mock_thread = GeneratedThreadResponse(
            topic="Open Source LLM Performance",
            hook_score=94,
            archetype="Framework",
            tweets=[
                "Open source models have crossed reasoning parity 🧵 1/2",
                "Here is the breakdown of inference latency • 2/2",
            ],
            items=[
                ThreadItemCreate(position=0, item_type="hook", text="Open source models have crossed reasoning parity 🧵 1/2"),
                ThreadItemCreate(position=1, item_type="body", text="Here is the breakdown of inference latency • 2/2"),
            ],
        )
        ...
        mock_container.browser.execute = AsyncMock(return_value=MagicMock(status="success"))

        with patch("xbot.ai.client.get_ai_client"), \
             patch("xbot.pipelines.trend_generator_pipeline.generate_visual_post_spec", AsyncMock(return_value=mock_visual_spec)), \
             patch("xbot.pipelines.trend_generator_pipeline.generate_poll", AsyncMock(return_value=mock_poll)), \
             patch("xbot.pipelines.trend_generator_pipeline.generate_thread", AsyncMock(return_value=mock_thread)), \
             patch("xbot.ai.image_engine.generate_post_image_async", AsyncMock(return_value={"file_path": "/tmp/test.png"})):
            result = await run_trend_generator_for_profile(db=db, profile=profile, guard=guard, max_items=5, container=mock_container)
