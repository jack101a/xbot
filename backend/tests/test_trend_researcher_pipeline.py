...
async def test_discover_candidate_trends():
    ...
    trend_res = BrowserResponse(
        status="success",
        action=BrowserActionType.SCRAPE_TRENDING,
        scrape=ScrapeResult(raw={"trends": [{"name": "#OpenAI", "category": "Technology"}]})
    )
    feed_res = BrowserResponse(
        status="success",
        ...
        scrape=ScrapeResult(
            tweets=[
                TweetData(
                    tweet_id="1234567890",
                    url="https://x.com/deepseek/status/1234567890",
                    handle="deepseek",
                    text="DeepSeek just dropped a breakthrough research paper on distillation.",
                    views=100000,
                )
            ]
        )
    )
    mock_browser.execute.side_effect = [trend_res, feed_res]
    ...
    with patch("xbot.pipelines.trend_researcher_pipeline.fetch_rss_trends", AsyncMock(return_value=[])):
        candidates = await discover_candidate_trends(profile, mock_redis, container=container)
