...
def test_persona_with_target_kols(tmp_path: Path) -> None:
    ...
    assert min_persona.stances == []
    assert min_persona.language_config.enable_hinglish is True
    assert min_persona.expressiveness_config.emoji_mode == "contextual_tone"


def test_worldview_and_linguistic_engine(tmp_path: Path) -> None:
    from xbot.persona import (
        EntityStance,
    ...
    persona_data = {
        ...
        "display_name": "Kaya",
        "x_handle": "@jackds1234",
        "identity": {"background": "Digital creator & cinephile"},
        "personality": {"communication_style": "Witty & observant"},
        "interests": {"primary": ["cinema", "anime", "tech"]},
        "writing_style": {"tone": "sharp", "typical_length": "short"},
        "goals": {},
        "rules": {"always": [], "never": []},
        "stances": [
            {
                ...
                "talking_points": ["practical effects", "70mm IMAX", "sound design"],
                "behavioral_rule": "Defend practical sets over CGI, acknowledge polarizing dialogue mixing.",
                "is_active": True,
            },
            {
                ...
                "category": "tech",
                "archetype": "nemesis",
                "sentiment_split": {"praise": 5, "analysis": 20, "critique": 75},
                "talking_points": ["lazy buzzwords", "untested claims"],
                "behavioral_rule": "Ruthlessly mock superficial hype and jargon.",
                "is_active": True,
            },
        ],
        "language_config": {
    ...
    p_dir = tmp_path / "kaya_profile"
    p_dir.mkdir()
    with (p_dir / "persona.yaml").open("w", encoding="utf-8") as f:
        yaml.dump(persona_data, f)

    persona = load_persona(p_dir)
    assert len(persona.stances) == 2
    assert persona.stances[0].archetype == "loyalist"
    assert persona.language_config.slang_register == "urban_buff"

    # 1. Test entity stance matching
    nolan_text = "Did you see Christopher Nolan is shooting his next movie on new IMAX 65mm cameras?"
    matches = match_entity_stances(nolan_text, persona)
    assert len(matches) == 1
    assert matches[0]["stance"].name == "Christopher Nolan"
    assert matches[0]["selected_angle"] in ["praise", "analysis", "critique"]

    # Alias matching (e.g. 'nolan')
    alias_text = "Honestly Nolan's practical stunts are unhinged."
    alias_matches = match_entity_stances(alias_text, persona)
    assert len(alias_matches) == 1
    assert alias_matches[0]["stance"].name == "Christopher Nolan"

    # 2. Test language detection (English vs Hinglish vs Devanagari)
    en_res = detect_language_and_script("Just updated my workstation with a new M4 chip.")
    assert en_res["dominant_vibe"] == "english"

    hinglish_res = detect_language_and_script("Yaar yeh camera quality sahi hai par battery backup bekaar hai bhai.")
    assert hinglish_res["dominant_vibe"] == "hinglish"
    assert hinglish_res["has_hinglish"] is True

    hindi_res = detect_language_and_script("यह फिल्म बहुत ही बेहतरीन है, निर्देशन कमाल का है।")
    assert hindi_res["dominant_vibe"] == "hindi_script"
    assert hindi_res["has_devanagari"] is True

    # 3. Test prompt section generation
    prompt_section = build_worldview_prompt_section(
        persona,
        context_text=nolan_text,
        top_comments=[{"text": "Bhai Nolan is the goat, scene hi alag hai"}],
        is_reply=True,
    )
    assert "ACTIVE ENTITY STANCE DETECTED" in prompt_section
    assert "Christopher Nolan" in prompt_section
    assert "EMOJI GRAMMAR & VISUAL EXPRESSIVENESS" in prompt_section
    assert "paralinguistic tone markers" in prompt_section.lower()
    assert "REACTION GIF/MEME ATTACHMENT" in prompt_section
