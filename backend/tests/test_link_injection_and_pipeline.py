...
async def test_growth_scorer_integration_skips_low_quality_targets(db_session: AsyncSession, sample_persona_with_kol: Persona):
    ...
    async def mock_browser_execute_stale(req: BrowserRequest) -> BrowserResponse:
        if req.action == BrowserActionType.CHECK_USER_LATEST:
            return BrowserResponse(
                status="success",
                action=req.action,
                action_result=ActionResult(status="success", target_id="999888777", raw=stale_tweet_data),
            )
        return BrowserResponse(status="failed", action=req.action, error="Unknown action")

    mock_container_stale = MagicMock()
    mock_container_stale.browser.execute = AsyncMock(side_effect=mock_browser_execute_stale)
    ...
    with patch("xbot.tasks.redis.from_url", return_value=mock_redis), \
         ...
         patch("xbot.container.get_container", return_value=mock_container_stale), \
...
async def test_growth_scorer_integration_executes_high_opportunity_target(db_session: AsyncSession, sample_persona_with_kol: Persona):
    ...
    async def mock_browser_execute_fresh(req: BrowserRequest) -> BrowserResponse:
        if req.action == BrowserActionType.CHECK_USER_LATEST:
            return BrowserResponse(
                status="success",
                action=req.action,
                action_result=ActionResult(status="success", target_id="1122334455", raw=fresh_tweet_data),
            )
    ...
    mock_container_fresh = MagicMock()
    mock_container_fresh.browser.execute = AsyncMock(side_effect=mock_browser_execute_fresh)
    ...
    with patch("xbot.tasks.redis.from_url", return_value=mock_redis), \
         ...
         patch("xbot.tasks.BrowserManager.acquire_lock", return_value=True), \
         patch("xbot.tasks.BrowserManager.release_lock", return_value=True), \
         patch("xbot.container.get_container", return_value=mock_container_fresh), \
        ...
        assert mock_container_fresh.browser.execute.call_count == 2
...
async def test_session_post_link_extraction_and_1st_reply_staging(db_session: AsyncSession, sample_persona_with_kol: Persona):
    ...
    with patch("xbot.tasks.AsyncSessionLocal", TestSessionLocal), \
         ...
         patch("xbot.tasks.SafetyGuard.is_action_safe", AsyncMock(return_value=True)), \
         patch("xbot.tasks.SafetyGuard.record_action_success", AsyncMock()), \
         patch("xbot.tasks.session_runner.scan_session_feed", AsyncMock(return_value=[])), \
         patch("xbot.tasks.BrowserManager.start", AsyncMock()), \
         patch("xbot.tasks.BrowserManager.stop", AsyncMock()), \
         patch("xbot.tasks.BrowserManager.acquire_lock", return_value=True), \
         patch("xbot.tasks.BrowserManager.release_lock", return_value=True), \
